...
const T = {
  ...
  orange: "#F5941E", orangeLt: "#ffa940", off: "#f0f4f8",
  gray: "#8899aa", txt: "#2a3545", sub: "#4a5568"
};
...
const globalCSS = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .fc { animation: fy 4s ease-in-out infinite; }
  .fc.f2 { animation-delay: 1s; }
  .fc.f3 { animation-delay: 2s; }
...
function Btn({ children, v = "primary", sm, full, onClick, style: sx }) {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      fontWeight: 600, fontSize: sm ? ".85rem" : ".95rem",
      padding: sm ? ".6rem 1.2rem" : ".85rem 2rem",
    ...
    <div style={{ position: "relative", width: "100%", aspectRatio: "1", display: "flex", alignItems: "center", justifyContent: "center" }}>
      {floats.map((f) => (
        <div key={f.cls} className={f.cls} style={{ position: "absolute", ...f.pos, background: "#fff", borderRadius: 12, padding: "12px 16px", boxShadow: "0 12px 40px rgba(0,0,0,.2)", display: "flex", alignItems: "center", gap: 10, zIndex: 3 }}>
          ...
          <div>
            <div style={{ fontSize: 12, fontWeight: 600, color: "#1a2a3d" }}>{f.t}</div>
            <div style={{ fontSize: 10, color: T.gray }}>{f.s}</div>
          </div>
        </div>
      ))}
...
function Section({ tag, title, sub, children, bg }) {
  return (
...
export default function MesColisLanding() {
  const [fp, setFp] = useState("");
  const [tp, setTp] = useState("");
  const [wt, setWt] = useState("");
  const [qt, setQt] = useState(null);
  const [st, setSt] = useState("Package");
  const [page, setPage] = useState("home");
  const [loginEmail, setLoginEmail] = useState("");
  const [loginPw, setLoginPw] = useState("");
  const [regForm, setRegForm] = useState({ fn: "", ln: "", em: "", pw: "", co: "", at: "Business" });

  const getQ = () => {
    if (!fp || !tp || !wt) return;
    const w = parseFloat(wt) || 1;
    setQt(CARRIERS.map(c => ({ ...c, p: +(c.p * w / 2).toFixed(2), lp: +(c.lp * w / 2).toFixed(2) })).sort((a, b) => a.p - b.p));
  };
    ...
    return (
      <div style={{ fontFamily: "'Outfit',sans-serif", background: `linear-gradient(135deg,${T.navy},#0a2e54)`, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ width: 400, background: "#fff", borderRadius: 16, padding: 36, boxShadow: "0 20px 60px rgba(0,0,0,.3)" }}>
          ...
          <Inp label="Email" type="email" placeholder="you@company.ca" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} />
          <Inp label="Password" type="password" placeholder="••••••••" value={loginPw} onChange={e => setLoginPw(e.target.value)} />
          <Btn full onClick={() => alert("This connects to POST /api/auth/login in the C# backend")}>Sign In</Btn>
          <p style={{ textAlign: "center", marginTop: 18, fontSize: 13, color: T.gray }}>
            No account?{" "}
            <span onClick={() => setPage("register")} style={{ color: T.blue, cursor: "pointer", fontWeight: 600 }}>Sign Up</span>
          </p>
          <p style={{ textAlign: "center", marginTop: 10 }}>
            <span onClick={() => setPage("home")} style={{ fontSize: 12, color: T.gray, cursor: "pointer" }}>← Back to home</span>
          </p>
        </div>
      </div>
    );
  ...
  // ─── REGISTER PAGE ───
  if (page === "register") {
    const u = (k, v) => setRegForm(p => ({ ...p, [k]: v }));
    return (
      <div style={{ fontFamily: "'Outfit',sans-serif", background: `linear-gradient(135deg,${T.navy},#0a2e54)`, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <style>{globalCSS}</style>
        <div style={{ width: 440, background: "#fff", borderRadius: 16, padding: 36, boxShadow: "0 20px 60px rgba(0,0,0,.3)" }}>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 18, justifyContent: "center" }}>
            {["Business", "Consumer"].map(t => (
              <button key={t} onClick={() => u("at", t)} style={{ padding: "8px 20px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, fontFamily: "'Outfit',sans-serif", background: regForm.at === t ? T.orange : T.off, color: regForm.at === t ? "#fff" : T.sub }}>{t}</button>
            ))}
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 14px" }}>
            <Inp label="First Name" value={regForm.fn} onChange={e => u("fn", e.target.value)} />
            <Inp label="Last Name" value={regForm.ln} onChange={e => u("ln", e.target.value)} />
          </div>
          <Inp label="Email" type="email" placeholder="you@company.ca" value={regForm.em} onChange={e => u("em", e.target.value)} />
          {regForm.at === "Business" && <Inp label="Company" value={regForm.co} onChange={e => u("co", e.target.value)} />}
          <Inp label="Password" type="password" placeholder="Min 8 characters" value={regForm.pw} onChange={e => u("pw", e.target.value)} />
          <Btn full onClick={() => alert("This connects to POST /api/auth/register in the C# backend")}>Create Account</Btn>
          <p style={{ textAlign: "center", marginTop: 18, fontSize: 13, color: T.gray }}>
            Have an account?{" "}
            <span onClick={() => setPage("login")} style={{ color: T.blue, cursor: "pointer", fontWeight: 600 }}>Sign In</span>
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ fontFamily: "'Outfit',sans-serif", background: "#fff", color: T.txt }}>
      ...
      <nav style={{ position: "fixed", top: 0, width: "100%", zIndex: 1000, background: "rgba(255,255,255,.95)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(0,0,0,.06)", boxShadow: "0 1px 20px rgba(0,0,0,.05)" }}>
        <div style={{ maxWidth: 1400, margin: "0 auto", padding: "0 2rem", display: "flex", justifyContent: "space-between", alignItems: "center", height: 64 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ width: 36, height: 36, background: `linear-gradient(135deg,${T.orange},${T.orangeLt})`, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
            {["Solutions", "How It Works", "Pricing", "Contact"].map(l => (
              <span key={l} style={{ fontSize: 14, fontWeight: 500, color: T.sub, cursor: "pointer" }}>{l}</span>
            ))}
            ...
            <Btn v="outline" sm onClick={() => setPage("login")}>Log In</Btn>
            <Btn sm onClick={() => setPage("register")}>Get Started</Btn>
          </div>
        </div>
      </nav>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 2rem", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center", position: "relative", zIndex: 2, width: "100%" }}>
          <div style={{ padding: "3rem 0" }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(245,148,30,.12)", border: "1px solid rgba(245,148,30,.2)", color: T.orangeLt, padding: "6px 16px", borderRadius: 100, fontSize: 12, fontWeight: 600, marginBottom: 24, animation: "fu .6s ease .2s both" }}>
              <span style={{ width: 6, height: 6, background: T.orange, borderRadius: "50%", animation: "blink 2s ease-in-out infinite", display: "inline-block" }} />
              Now Launching in Quebec
            </div>
            <h1 style={{ fontSize: "clamp(2.8rem,5vw,4rem)", fontWeight: 800, color: "#fff", lineHeight: 1.1, marginBottom: 24, animation: "fu .6s ease .35s both" }}>
              Canada's First<br />
              <span style={{ background: `linear-gradient(135deg,${T.orange},#ffcc4d)`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Smart B2B Locker</span>
              <br />Network
            </h1>
            <p style={{ fontSize: 17, lineHeight: 1.7, color: "rgba(255,255,255,.65)", maxWidth: 480, marginBottom: 32, animation: "fu .6s ease .5s both" }}>
              Carrier-agnostic, 24/7 locker-to-locker delivery infrastructure for couriers, 3PLs, and cross-border commerce. Purpose-built for Canada.
            </p>
            <div style={{ display: "flex", gap: 16, animation: "fu .6s ease .65s both" }}>
              <Btn onClick={() => setPage("register")}>Start Free Pilot →</Btn>
            ...
            <div style={{ display: "flex", gap: 40, marginTop: 40, paddingTop: 24, borderTop: "1px solid rgba(255,255,255,.08)", animation: "fu .6s ease .8s both" }}>
              {[["99%", "First-attempt delivery"], ["24/7", "Pickup & drop-off"], ["40%", "Lower last-mile cost"]].map(([n, l]) => (
                <div key={n}>
                  <div style={{ fontSize: 28, fontWeight: 800, color: T.orange, lineHeight: 1 }}>{n}</div>
                  <div style={{ fontSize: 12, color: "rgba(255,255,255,.45)", marginTop: 4 }}>{l}</div>
                </div>
              ))}
            </div>
          </div>
          <div style={{ animation: "fu .8s ease .4s both" }}>
            <LockerVisual />
          </div>
        </div>
      </section>
      ...
      <div style={{ padding: "2.5rem 0", borderBottom: "1px solid rgba(0,0,0,.05)", background: T.off }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", textAlign: "center" }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 50, opacity: 0.35 }}>
            {["ShipTime", "DHL", "Zonos", "Purolator", "Canada Post", "UPS"].map(n => (
              <span key={n} style={{ fontSize: 18, fontWeight: 800, letterSpacing: 1, color: T.txt }}>{n}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      {/* HOW IT WORKS */}
      <Section tag="How It Works" title="Ship smarter in 4 simple steps" sub="From label creation to locker pickup — our platform handles the entire delivery flow.">
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 32, position: "relative" }}>
          ...
          {[
            ["1", "Connect via API", "Integrate your shipping platform with our open REST API in minutes."],
            ["2", "Generate Shipment", "Create labels, assign locker destinations, get real-time rates."],
            ["3", "Drop at Locker", "Courier drops at nearest smart locker. 99% first-attempt success."],
            ["4", "Pickup 24/7", "Recipient gets SMS + email with secure QR code or PIN."],
          ].map(([num, title, desc]) => (
      ...
      {/* FEATURES */}
      <Section tag="Platform Features" title="Everything you need for modern last-mile logistics" sub="Built for couriers, 3PLs, and e-commerce businesses." bg={T.off}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 20 }}>
          {[
            ["🌐", "#dbeafe", "Carrier-Agnostic Network", "Multi-carrier utilization improves network density and ROI."],
            ["📦", "#d1fae5", "Locker-to-Locker", "Micro-hub distribution, consolidated drops, and returns."],
            ["🌍", "#ffedd5", "Cross-Border Ready", "Zonos-powered duties, taxes, and landed cost calculations."],
            ["🔌", "#e0e7ff", "Open REST API", "Integrates with any courier, 3PL, or e-commerce system."],
            ["🇨🇦", "#fce7f3", "Bilingual (FR/EN)", "Native French and English interfaces and notifications."],
            ["📊", "#fef3c7", "Real-Time Analytics", "Track utilization, delivery rates, and revenue."],
          ].map(([ic, bg, t, d]) => (
          ...
          <div style={{ background: "#fff", border: "1px solid rgba(0,0,0,.06)", borderRadius: 16, padding: 28, boxShadow: "0 4px 24px rgba(0,0,0,.06)" }}>
            <div style={{ display: "flex", gap: 6, marginBottom: 20 }}>
              {["Package", "Envelope", "LTL Freight", "Locker-to-Locker"].map(t => (
                <button key={t} onClick={() => setSt(t)} style={{ padding: "8px 16px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, fontFamily: "'Outfit',sans-serif", background: st === t ? T.orange : T.off, color: st === t ? "#fff" : T.sub }}>{t}</button>
              ))}
            </div>
            ...
            {qt && (
              <div style={{ marginTop: 20 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: T.gray, marginBottom: 14 }}>
                  {qt.length} options — save up to {Math.round((1 - qt[0].p / qt[0].lp) * 100)}%
                </div>
                {qt.map((q, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 18px", borderRadius: 12, marginBottom: 6, background: i === 0 ? "rgba(245,148,30,.08)" : T.off, border: i === 0 ? "1px solid rgba(245,148,30,.25)" : "1px solid transparent" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <span style={{ fontSize: 22 }}>{q.l}</span>
                      <div>
                        <div style={{ fontWeight: 600, fontSize: 14, color: T.navy }}>{q.n} — {q.s}</div>
                        <div style={{ fontSize: 12, color: T.gray }}>{q.d} day{q.d > 1 ? "s" : ""}</div>
                      </div>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                      <div style={{ textAlign: "right" }}>
                        <div style={{ fontSize: 11, color: T.gray, textDecoration: "line-through" }}>${q.lp.toFixed(2)}</div>
                        <div style={{ fontSize: 20, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace", color: i === 0 ? T.orange : T.navy }}>${q.p.toFixed(2)}</div>
                      </div>
                      <Btn sm onClick={() => setPage("register")}>{i === 0 ? "Best Rate →" : "Select"}</Btn>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {/* PRICING */}
      <Section tag="Pricing" title="Simple, transparent pricing" sub="Start free during our pilot phase. Scale with volume-based pricing.">
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 24, maxWidth: 1000, margin: "0 auto" }}>
          {[
            { n: "Starter", d: "Small businesses", p: "$0", per: "Pilot phase", f: ["Up to 50 drops/mo", "API access", "Bilingual notifications", "Basic analytics"], ft: false },
            { n: "Growth", d: "Carriers & 3PLs", p: "$299", per: "Per month + usage", f: ["Unlimited drops", "Priority API", "Account manager", "Advanced analytics", "Custom SLAs"], ft: true },
            { n: "Enterprise", d: "National carriers", p: "Custom", per: "Volume pricing", f: ["White-label program", "Dedicated infra", "Custom integrations", "24/7 support", "Co-designed workflows"], ft: false },
          ].map(p => (
            <div key={p.n} style={{ borderRadius: 16, padding: "2.5rem 2rem", border: p.ft ? "none" : "1px solid rgba(0,0,0,.06)", background: p.ft ? `linear-gradient(135deg,${T.navy},#0a2e54)` : "#fff", color: p.ft ? "#fff" : T.txt, boxShadow: p.ft ? "0 20px 60px rgba(11,20,38,.3)" : "none", position: "relative", overflow: "hidden", transition: "transform .4s" }}>
              {p.ft && <div style={{ position: "absolute", top: 0, right: 0, background: T.orange, color: "#fff", fontSize: 10, fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", padding: "5px 16px", borderBottomLeftRadius: 10 }}>Popular</div>}
              <div style={{ fontSize: 18, fontWeight: 700, color: p.ft ? "#fff" : T.navy, marginBottom: 6 }}>{p.n}</div>
              <div style={{ fontSize: 13, color: p.ft ? "rgba(255,255,255,.6)" : T.gray, marginBottom: 20 }}>{p.d}</div>
              <div style={{ fontSize: 40, fontWeight: 900, color: p.ft ? T.orange : T.navy, lineHeight: 1, marginBottom: 4 }}>
                {p.p}<span style={{ fontSize: 14, fontWeight: 500 }}>{p.p !== "Custom" ? " / mo" : ""}</span>
              </div>
              <div style={{ fontSize: 13, color: p.ft ? "rgba(255,255,255,.5)" : T.gray, marginBottom: 24 }}>{p.per}</div>
              {p.f.map(f => (
                <div key={f} style={{ fontSize: 14, color: p.ft ? "rgba(255,255,255,.7)" : T.sub, display: "flex", alignItems: "center", gap: 8, paddingBottom: 10, marginBottom: 10, borderBottom: `1px solid ${p.ft ? "rgba(255,255,255,.08)" : "rgba(0,0,0,.04)"}` }}>
                  <span style={{ color: "#059669", fontWeight: 800, fontSize: 12 }}>✓</span> {f}
                </div>
              ))}
              <Btn v={p.ft ? "primary" : "outline"} full style={{ marginTop: 8 }} onClick={() => setPage("register")}>
                {p.ft ? "Start Growth Plan" : p.n === "Enterprise" ? "Contact Sales" : "Get Started Free"}
              </Btn>
            </div>
          ))}
        </div>
      ...
      <section style={{ padding: "5rem 0", background: `linear-gradient(135deg,${T.navy},#0a2e54,#0e3b6e)`, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ textAlign: "center", position: "relative", zIndex: 2, maxWidth: 700, margin: "0 auto", padding: "0 2rem" }}>
          ...
          <p style={{ fontSize: 16, color: "rgba(255,255,255,.6)", marginBottom: 32 }}>Join the pilot program. Deploy smart lockers. Cut costs by 40%.</p>
          <div style={{ display: "flex", gap: 16, justifyContent: "center" }}>
            <Btn onClick={() => setPage("register")}>Start Free Pilot →</Btn>
            <Btn v="ghost">Contact Sales</Btn>
          </div>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 2rem" }}>
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: 32, marginBottom: 40 }}>
            <div>
              ...
              <p style={{ fontSize: 14, lineHeight: 1.7, maxWidth: 280 }}>Canada's first open, carrier-agnostic smart locker network for B2B last-mile logistics.</p>
            </div>
            {[
              { t: "Product", l: ["Solutions", "How It Works", "API Docs", "Pricing"] },
              { t: "Company", l: ["About", "Blog", "Careers", "Contact"] },
              { t: "Legal", l: ["Privacy Policy", "Terms", "Cookies"] },
            ].map(c => (
              <div key={c.t}>
                <h4 style={{ fontSize: 11, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", color: "rgba(255,255,255,.3)", marginBottom: 16 }}>{c.t}</h4>
                {c.l.map(l => (
                  <a key={l} href="#" style={{ display: "block", fontSize: 14, color: "rgba(255,255,255,.55)", textDecoration: "none", marginBottom: 10 }}>{l}</a>
                ))}
              </div>
            ))}
          </div>
          <div style={{ borderTop: "1px solid rgba(255,255,255,.06)", paddingTop: 20, display: "flex", justifyContent: "space-between", fontSize: 13 }}>
            <span>© 2026 MesColis Inc. All rights reserved.</span>
            <span>pilot@mescolis.ca</span>
          </div>
        </div>
      </footer>
    </div>
